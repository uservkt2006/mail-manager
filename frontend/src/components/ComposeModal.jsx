  ...
  return (
    <div className="fixed bottom-6 right-6 w-[520px] max-w-[92vw] bg-dark-surface border border-dark-border rounded-xl shadow-2xl z-50 flex flex-col max-h-[80vh]">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-dark-border">
        <h2 className="text-sm font-semibold text-ink-strong">{replyTo ? 'Trả lời' : 'New Mail'}</h2>
      ...
      <div className="px-4 py-3 space-y-2 border-b border-dark-border">
        <input value={to} onChange={e => setTo(e.target.value)} placeholder="To" disabled={!!replyTo}
          className="w-full bg-transparent border border-dark-border rounded-md px-3 py-2 text-sm text-ink-strong placeholder-gray-600 focus:outline-none focus:border-primary disabled:opacity-60" />
        <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="Subject"
          className="w-full bg-transparent border border-dark-border rounded-md px-3 py-2 text-sm text-ink-strong placeholder-gray-600 focus:outline-none focus:border-primary" />
      </div>
      <textarea value={body} onChange={e => setBody(e.target.value)} placeholder="Nội dung…" rows={8}
        className="flex-1 bg-transparent px-4 py-3 text-sm text-ink placeholder-gray-600 focus:outline-none resize-none min-h-[140px]" />
      ...
      <div className="flex items-center justify-between px-4 py-3 border-t border-dark-border">
        <button className="p-1.5 rounded hover:bg-dark-hover text-ink-dim" title="Đính kèm (qua Exchange khi kết nối tài khoản)"><Paperclip size={15} /></button>
