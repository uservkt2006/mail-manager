      ...
      <div className="p-3 border-b border-dark-border">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-0.5">
            <button onClick={onToggleConversation} title="Chế độ hội thoại"
              className={`p-1.5 rounded hover:bg-dark-hover ${conversationView ? 'text-primary' : 'text-ink-dim'}`}>
              <MessageSquare size={14} />
            </button>
            <button onClick={onRefresh} className="p-1.5 rounded hover:bg-dark-hover text-ink-dim"><RefreshCw size={14} /></button>
          </div>
        </div>
        <div className="relative mb-2">
          ...
          <input value={searchTerm} onChange={e => onSearchChange(e.target.value)} placeholder="Lọc trong thư mục này…"
            className="w-full bg-dark-bg border border-dark-border rounded-md pl-8 pr-3 py-1.5 text-xs text-ink placeholder-gray-600 focus:outline-none focus:border-primary" />
        </div>
        <div className="flex flex-wrap gap-1">
          <button onClick={() => onCategoryChange('')}
            className={`px-2 py-0.5 rounded text-[11px] ${selectedCategory === '' ? 'bg-primary text-ink-strong' : 'bg-dark-bg text-ink-dim hover:bg-dark-hover'}`}>
            Tất cả
          </button>
          ...
          return (
            <div key={email.id} onClick={() => onEmailSelect(email)}
              ...
              <div className="flex items-start justify-between mb-0.5 gap-2">
                <span className={`text-[13px] truncate flex-1 ${unread ? 'text-ink-strong font-semibold' : 'text-ink-dim'}`}>
              ...
              <div className={`text-[13px] truncate ${unread ? 'text-ink font-medium' : 'text-ink-dim'}`}>{email.subject}</div>
              ...
              <div className="flex items-center gap-1.5 mt-1.5">
                ...
                <div className="opacity-0 group-hover:opacity-100 flex gap-1 transition-opacity">
                  ...
                  <button onClick={e => { e.stopPropagation(); onArchive(email.id) }} title="Lưu trữ (A)"><Archive size={12} className="text-ink-mute hover:text-primary" /></button>
